...
#define PAGE_SIZE 4096 /* XXX in system header file */
...
static int get_first_free(struct page *page)
{
	for (size_t i = 0; i < BLOCKS_SIZE; ++i)
	{
		...
		/* XXX panic("block != -1 but no bit found"); */
	}
	return -1;
}
